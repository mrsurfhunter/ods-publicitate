...
function CatalogCard({ pkg, onPurchased }) {
  ...
  return (
    <div className={`catalog-card ${pkg.pop ? 'popular' : ''}`}>
      {pkg.pop && <div className="catalog-card-pop-badge">RECOMANDAT</div>}
          ...
          <div className="catalog-details">
            {pkg.inc.map((x, i) => (
              <div key={i} className="catalog-detail-row">
                <span style={{ color: pkg.color, fontWeight: 800, flexShrink: 0 }}>✓</span>
                <div>
                  <span style={{ fontWeight: 700, color: 'var(--c-navy)' }}>{x.w}</span>
  ...
  return (
    <div className="catalog view-enter">
      <div className="catalog-header">
        <h2 className="heading-lg" style={{ color: 'var(--c-navy)', marginBottom: 8 }}>Toate pachetele</h2>
